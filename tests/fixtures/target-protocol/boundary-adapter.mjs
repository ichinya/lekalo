...
const adapter = { id:'test', version:'0.2.16', digest:digest('boundary adapter v1') };
const file = arg('--lekalo-request-file');
const req = JSON.parse(fs.readFileSync(file ?? 0, 'utf8'));
const res = { protocol:'lekalo.target/v1', protocol_version:'0.2.16', operation:req.operation, request_id:req.request_id, status:'ok', evidence:{adapter} };
const caps = { adapter, protocol_versions:['0.2.16'], ir_versions:['0.2.16'], operations:['describe','scan','bind','validate','verify','generate','plan-clean','clean'], transports:['stdin','file'], targets:['test'], profiles:['default','other'], read_scopes:['.lekalo/ir/**'], write_scopes:['out/**'], progress:false };
