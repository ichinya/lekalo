 ...
 * `_testkit.ts`, the run-record reporter `_reporter.mjs`, and the port
 * shim `_port.ts`, each test paired with one canonical `.map.json`
 * sidecar.
 *
...
export const SCENARIO_DIR = ".lekalo/generated/scenario-tests";
...
export const RESERVED_MODULES = Object.freeze(["_testkit", "_port", "_reporter"]);
...
export function emitScenarioTests(input) {
  ...
  const files = [
    file(`${SCENARIO_DIR}/_testkit.ts`, testkitText(context)),
    file(`${SCENARIO_DIR}/_reporter.mjs`, reporterText(context)),
    file(`${SCENARIO_DIR}/_port.ts`, portText(context)),
  ];
...
// the root is four levels up: .lekalo/generated/scenario-tests/<module>/<file>.
const GENERATED_ROOT_DEPTH = 4;
...
function portText(context) {
  ...
  // port lives (plan §4).
  const depth = SCENARIO_DIR.split("/").length; // .lekalo + generated + scenario-tests
  const ups = Array.from({ length: depth }, () => "..").join("/");
  const specifier = `${ups}/${context.portModulePath}`;
  return `${docHeader(context, " (port binding shim)").join("\n")}
//
// The project test-port binding shim.
...
//
// Scenario ${model.id} @${model.version}: ${model.summary}
// Runner ${model.runner.id}; binding ${model.binding.mode}; generated by
// the scenario-test compiler (issue #47).

import assert from "node:assert/strict";
import { test } from "node:test";
import { port, resetPort } from "../_port.ts";
import {
  boundedDetail,
  errorFieldsMatch,
  typedEqual,
} from "../_testkit.ts";
import { createRunRecorder } from "../_reporter.mjs";
      ...
      return [
        ...
        ...(Object.keys(exactFields).length > 0
          ? [
              ...
              `  Object.fromEntries(Object.entries(row).filter(([key]) => Object.hasOwn(${JSON.stringify(Object.keys(exactFields))}, key))),`,
