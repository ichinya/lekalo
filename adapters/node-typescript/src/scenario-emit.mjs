...
function renderChecks(step, model, stepVars, clockIsos, observed) {
  ...
  switch (step.kind) {
    ...
    case "entity_state": {
      ...
      // The closed matcher vocabulary compiles to the exact canonical-
      // form checks (review F-3, R-2): the datetime/uri/decimal checks
      // route through the shared testkit helpers — exact ports of the
      // core grammar functions in scenario/value.rs — so a stored state
      // field that violates the canonical contract fails the matcher
      // instead of over-accepting; uuid/non-null are exact.
      const matchCheck = {
