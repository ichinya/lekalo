...
import { descriptor as generationComposite } from "./generation-composite.mjs";
...
kernel.__setLaunchExtensions([
  ...
  // The closed wire has exactly one generate operation: the Zod and
  // transport generators compose under the composite descriptor
  // (issues #45 and #70) instead of claiming the operation twice.
  generationComposite,
]);
...
async function buildZodArtifact() {
  ...
  for (const name of ["zod-gen.mjs", "zod-map.mjs", "zod-emit.mjs", "zod-policy.mjs"]) {
    writeFileSync(
      join(zodScratchRoot, "src", name),
      readFileSync(join(adapterRoot, "src", name), "utf8").replace(stripShebang, ""),
    );
  }
  const zodEntry = `// Generated by build.mjs — the Zod generation artifact entry.
import * as kernel from "./kernel.mjs";
import { descriptor as zodDescriptor } from "./zod-gen.mjs";
...
`;
  ...
  writeFileSync(
    join(zodScratchRoot, "package.json"),
    JSON.stringify({ name: "lekalo-adapter-zod-build", type: "module", private: true }),
  );
  const result = await build({
    entryPoints: [join(zodScratchRoot, "src", "main.mjs")],
    bundle: true,
    platform: "node",
    format: "esm",
    target: "node18",
  ...
  const zodBanner = `#!/usr/bin/env node
/**
...
// The issue #45 generation artifact: the kernel plus the Zod generator,
// without the vendored compiler or the scanner. Self-contained, well
// under the protocol's 4 MiB entry bound, and byte-stable.
const zodArtifact = await buildZodArtifact();
const zodDigest = "sha256:" + createHash("sha256").update(zodArtifact, "utf8").digest("hex");
...
if (check) {
  ...
  const zodPath = join(adapterRoot, "adapter-zod.mjs");
  if (!existsSync(zodPath)) {
    process.stderr.write("--check: committed adapter-zod.mjs is missing\n");
    process.exit(1);
  }
  if (readFileSync(zodPath, "utf8") !== zodArtifact) {
    process.stderr.write(
      `--check: committed adapter-zod.mjs differs from the deterministic rebuild\n` +
        `rebuild digest: ${zodDigest}\n`,
    );
    process.exit(1);
  }
  process.stdout.write(
    JSON.stringify({ ok: true, check: true, digest, zodDigest, bytes: Buffer.byteLength(artifact) }) + "\n",
  );
} else {
  writeFileSync(artifactPath, artifact);
  writeFileSync(join(adapterRoot, "adapter-zod.mjs"), zodArtifact);
  process.stdout.write(
    JSON.stringify({
      ok: true,
