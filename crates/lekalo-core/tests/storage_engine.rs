...
/// The profile rebound to the migration base attachment: the planner
/// scenario keeps its own base document (the laravel+postgres
/// projection pair), so the binding digest is derived from it exactly
/// the way an authoring step would rebind it.
fn migration_profile() -> StorageEngineAttachment {
    let mut value: serde_json::Value = serde_json::from_slice(PROFILE).expect("profile json");
    let base = migration_attachment(MIGRATION_BASE);
    let base_bytes = base.canonical_bytes().expect("canonical");
    value["projectionRef"] = serde_json::Value::String(format!(
        "sha256:{}",
...
fn a_plan_under_a_refusing_array_policy_refuses_and_never_renders_the_type() {
    ...
    let base_bytes = migration_attachment(MIGRATION_BASE)
        .canonical_bytes()
        .expect("canonical");
    profile_value["projectionRef"] = serde_json::Value::String(format!(
        "sha256:{}",
...
fn a_destructive_plan_is_gated_and_blocked_until_the_plan_id_is_named() {
    ...
    let plan =
        lekalo_core::storage_engine::plan_migration(&migration_profile(), &base, &candidate, None)
            .expect("plans");
    ...
    let confirmed = lekalo_core::storage_engine::plan_migration(
        &migration_profile(),
        &base,
        &candidate,
        Some(&plan_id),
