...
fn legacy_adapter_stays_on_the_frozen_base_contract() {
    ...
    assert!(!discovered.ir_compatible("0.2.16"));
    // The full #27 operation round trip still works on the 0.2.16 session.
    let outcome = client
        .call(
            &command,
            CallRequest {
...
fn incompatible_adapter_is_filtered_before_any_ir_is_transferred() {
    ...
    assert!(
        !discovered.ir_compatible("0.2.16"),
...
fn discovery_cache_invalidates_on_version_and_digest_changes() {
    ...
    let key = Cache::key(&first, "0.2.16");
...
fn discovered_capabilities_resolve_into_the_lock_snapshot() {
    ...
    let model = ContractVersion::parse_canonical("0.2.16").unwrap();
    let core =
    ...
    let adapter_ref = ComponentRef::new(
        ...
        lekalo_core::lockfile::SemVer::parse("0.2.16").unwrap(),
    );
...
#[test]
fn resolved_profiles_reach_current_adapters() {
    ...
    // negotiates 0.2.16 and accepts the resolved profile members.
    ...
    let outcome = client
        ...
        .expect("a 0.2.16 adapter receives the resolved capabilities");
    assert_eq!(outcome.response.operation, Operation::Scan);
}
