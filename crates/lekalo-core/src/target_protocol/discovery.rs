...
mod tests {
    ...
    fn ir_compatibility_requires_exact_declared_membership() {
        let adapter = sample_adapter("sha256:aa", &["0.3.1"]);
        assert!(adapter.ir_compatible("0.3.1"));
        assert!(!adapter.ir_compatible("0.2.0"));
        let undeclared = sample_adapter("sha256:aa", &[]);
        assert!(
            !undeclared.ir_compatible("0.3.1"),
    ...
    fn profile_selection_is_deterministic() {
        let mut adapter = sample_adapter("sha256:aa", &["0.3.1"]);
    ...
    fn cache_hits_only_on_the_exact_version_and_digest_key() {
        ...
        let mut different_digest = sample_adapter("sha256:bb", &["0.3.1"]);
        ...
        let key3 = Cache::key(&sample_adapter("sha256:aa", &["0.3.1"]), "0.0.9");
        assert!(cache.get(&key3).is_none(), "a changed IR version misses");

        let mut changed_executable = sample_adapter("sha256:aa", &["0.3.1"]);
