//! Storage-engine-profile diagnostics routed through the accepted #11
//! contract (issue #117).
//!
//! Every profile failure is one registered `storage.profile-*` rule
...
/// The registered rule for fatal profile input violations.
pub(crate) const INPUT_INVALID: &str = "storage.profile-invalid";
/// The registered rule for the canonical profile payload bound.
pub(crate) const EXPORT_LIMIT: &str = "storage.profile-limit";
/// The registered rule for an impossible profile comparison.
pub(crate) const DIFF_INVALID: &str = "storage.profile-diff-invalid";
...
/// The fatal set for one semantic profile violation with a fixed detail
/// tag and an optional bounded subject echo.
pub(crate) fn rule_invalid(detail: &str, subject: Option<&str>) -> DiagnosticSet {
    let mut data = DataObject::new();
    data.insert("detail".to_owned(), token(detail));
...
/// The fatal set for one profile comparison refusal.
