//! Confined registry persistence (issue #40).
//!
//! The only writer in the contracted seam. Every write creates its
...
/// logical POSIX path (for example `.lekalo/import/contracted`); every
...
pub(crate) fn write_confined(
    ...
    let canonical_root = root
        .canonicalize()
        .map_err(|_| diagnostic::registry_io_set("root-unreadable"))?;
        ...
        .filter(|segment| !segment.is_empty())
    {
        ...
        match std::fs::symlink_metadata(&prefix) {
            Ok(metadata) => {
                if is_link(&metadata) {
                    return Err(diagnostic::registry_io_set("path-link"));
                }
                if !metadata.is_dir() {
                    return Err(diagnostic::registry_io_set("path-not-directory"));
                }
            }
            Err(_) => {
                std::fs::create_dir(&prefix)
                    .map_err(|_| diagnostic::registry_io_set("directory-create"))?;
            }
        }
    }
    ...
    std::fs::write(&temporary, bytes).map_err(|_| diagnostic::registry_io_set("registry-write"))?;
    std::fs::rename(&temporary, &target)
        .map_err(|_| diagnostic::registry_io_set("registry-rename"))?;
    Ok(())
}
