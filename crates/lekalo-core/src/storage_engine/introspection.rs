//! Normalization of the adapter-produced storage-observation
//! evidence (issue #69).
//!
...
fn from_value(json: &Json) -> Result<IntrospectionEvidence, DiagnosticSet> {
    ...
    if object.get("schemaVersion").and_then(Json::as_str)
        != Some("lekalo/storage-observation/v0.4.0")
    {
        return Err(diagnostic::input_invalid("schema-version"));
    }
    if object.get("identity").and_then(Json::as_str) != Some("dev.lekalo.storage-observation@0.4.0")
    {
...
fn payload(evidence: &IntrospectionEvidence) -> String {
    ...
    object(vec![
        (
            "schemaVersion",
            Some(string("lekalo/storage-observation/v0.4.0")),
        ),
        (
            "identity",
            Some(string("dev.lekalo.storage-observation@0.4.0")),
        ),
        (
            "evidenceRevision",
