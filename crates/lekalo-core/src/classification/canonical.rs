//! Canonical bytes of the classification contracts (issue #87).
//!
//! Attachment canonical bytes are compact UTF-8 JSON with byte-sorted
//! object keys, canonical collections, and no trailing LF. The same
//! rule is proven independently by the Node contract suite and by the
//! digest sidecars of the committed goldens.
...
/// is the attachment canonical form.
pub(super) fn canonical_value_bytes<T: Serialize>(value: &T) -> String {
    let dynamic = serde_json::to_value(value).expect("classification wire values serialize");
