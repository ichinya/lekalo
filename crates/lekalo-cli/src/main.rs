...
fn main() -> ExitCode {
    // The clap derive builds one deep command tree at startup, and the
    // debug build's frames overflow the OS default main-thread stack as
    ...
    match std::thread::Builder::new()
        ...
        .spawn(main_worker)
    {
        Ok(worker) => worker.join().unwrap_or_else(|_| std::process::exit(101)),
        Err(_) => std::process::exit(101),
    }
}

fn main_worker() -> ExitCode {
