...
2. **Scan.** Merge one pinned adapter scan document:
   ...
   (`tests/fixtures/pilot/taskhub-scans/`; the script re-runs the real
   node-typescript scanner kernel over the fixture and must reproduce
   the committed bytes exactly). Provenance boundary: `stableKey`,
   `location`, `fingerprint`, `candidates`, `mappingConfidence`, the
   adapter identity, and the `revision` are kernel-derived; the closed
   `kind`, the structural `evidence` (fields/values/identity and the
   resolved reference rows), the snake_cased semantic `id`s, and the
   endpoint rows are authored to the #39 evidence grammar from the
   fixture sources — the transport itself cannot carry this document
   (128-byte detail tokens; module-granular kernel edges), and the
   script records exactly where the one ends and the other begins.
