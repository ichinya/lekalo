// One-off issue #44 section-9 regeneration recipe (plan section 9).
// Repeats the accepted #43 reserve meaning for the 0.3.1 candidate:
...
// authorized implementation, from the repository root.
...
assert.equal(JSON.parse(execFileSync(bin, ['--version', '--json'],
  { encoding: 'utf8', cwd: repo })).version, '0.3.1');
const work = realpathSync(mkdtempSync(join(tmpdir(), 'lekalo-reserve-031-')));
...
assert.equal(lock.core.version, '0.3.1');
