...
const schemas = new Map([
  ...
  ["storage-observation", readJson("contracts/storage-observation.schema.v0.4.0.json")],
...
for (const name of ["observed.json", "observed-drifted.json"]) {
  ...
  const document = JSON.parse(text);
  if (!validators.get("storage-observation")(document)) {
    fail(`introspection:${name}:schema`, validators.get("storage-observation").errors);
    continue;
  }
  if (canonicalJson(document) !== text.trim()) {
...
for (const name of readdirSync(resolve(root, invalidDir)).sort()) {
  ...
  const [family, document] = name.startsWith("introspection-")
    ? ["storage-observation", readJson(`${invalidDir}/${name}`)]
