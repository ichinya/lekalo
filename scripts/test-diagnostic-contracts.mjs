...
const registry = read("contracts/diagnostic-registry.v1.18.0.json");
...
// observed family as 1.16.0, issue #30 adds the implementation family as
// the reserved 1.18.0; 1.17.0 stays reserved by its parallel owner).
...
// checks; registry116 additionally carries the family.
...
const implementationAdditions = registry.entries.filter((entry) => !registry116.entries.some((old) => old.id === entry.id));
if (
...
const implementation = registry116.entries.find((entry) => entry.code.startsWith("LEK-OBS-"));
const missing118 = structuredClone(registry);
missing118.entries = missing118.entries.filter((entry) => entry.id !== implementation.id);
const changed118 = structuredClone(registry);
changed118.entries.find((entry) => entry.id === implementation.id).allowed_statuses = ["valid"];
if (isAdditive(missing118, registry116) || isAdditive(changed118, registry116)) fail("additive-negative-control");
