...
const registry = read("contracts/diagnostic-registry.v1.13.0.json");
...
// adoption rules). Each successor is additive to its exact frozen
// predecessor: every predecessor entry and its semantics survive verbatim,
// and each step adds exactly its own family.
...
if (!isAdditive(registry, registry112)) fail("predecessor-entry-drift");
const initAdditions = registry.entries.filter((entry) => !registry112.entries.some((old) => old.id === entry.id));
