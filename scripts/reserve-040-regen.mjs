// One-off issue #45 review-round reserve regeneration recipe (the F-4
// version-custody fix). Repeats the accepted #85 reserve meaning for the
// 0.4.0 candidate on this branch: asserts the freshly built binary,
// regenerates the contract-only lock golden plus the two digest constants
// and the orchestration receipts, all inside new external disposable
// directories. Run only during authorized implementation, from the
// repository root. The branch carries the changed contracted-declaration
// contract family at 0.4.0 (see contracts/); the target-protocol family
// remains 0.3.2 and the unchanged families stay frozen.
...
assert.equal(JSON.parse(execFileSync(bin, ['--version', '--json'],
  { encoding: 'utf8', cwd: repo })).version, '0.4.0');
const work = realpathSync(mkdtempSync(join(tmpdir(), 'lekalo-reserve-040-')));
...
assert.equal(lock.core.version, '0.4.0');
